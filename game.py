...
def drawBoard(board):
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            pygame.draw.rect(screen, GREEN2, (c*SQUARESIZE, r *
                             SQUARESIZE+SQUARESIZE, SQUARESIZE, SQUARESIZE))
            pygame.draw.circle(screen, GREEN1, (int(
                c*SQUARESIZE+SQUARESIZE/2), int(r*SQUARESIZE+SQUARESIZE+SQUARESIZE/2)), RADIUS)

    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            if board[r][c] == 1:
                pygame.draw.circle(screen, ROBINEGG, (int(
                    c*SQUARESIZE+SQUARESIZE/2), height-int(r*SQUARESIZE+SQUARESIZE/2)), RADIUS)
            elif board[r][c] == 2:
                pygame.draw.circle(screen, LTPURPLE, (int(
                    c*SQUARESIZE+SQUARESIZE/2), height-int(r*SQUARESIZE+SQUARESIZE/2)), RADIUS)
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            pygame.draw.rect(screen, GREEN1, (0, 0, width, SQUARESIZE))
            posx = event.pos[0]
            if turn == 0:
                pygame.draw.circle(
                    screen, ROBINEGG, (posx, int(SQUARESIZE/2)), RADIUS)
            else:
                pygame.draw.circle(
                    screen, LTPURPLE, (posx, int(SQUARESIZE/2)), RADIUS)
        pygame.display.update()
            ...
            if turn == 0:
                ...
                if checkValid(board, col):
                    ...
                    if winCondition(board, 1):
                        label = myfont.render("Player 1 wins!!", 1, ROBINEGG)
            ...
            # # Ask for Player 2 Input
                ...
                if checkValid(board, col):
                    ...
                    if winCondition(board, 2):
                        label = myfont.render("Player 2 wins!!", 1, LTPURPLE)
                        screen.blit(label, (40, 10))
                        game_over = True
